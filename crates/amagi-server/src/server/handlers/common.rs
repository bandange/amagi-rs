...
use super::support::ApiSpecResult;
use super::{
    ApiSpecErrorResponse, ApiSpecResponse, DownstreamNodeSummary, HealthResponse,
    NodeHealthSummary, NodeSummary, PlatformApiSpecResponse, PlatformRouteSource, PlatformSummary,
...
const TWITTER_ENDPOINTS: &[&str] = &[
    ...
    "/api/twitter/user/{screen_name}/live-room-info",
    "/api/twitter/user-id/{user_id}/live-room-info",
    "/api/twitter/live-room/{broadcast_id}/stream",
    "/api/twitter/live-media/{media_key}/stream",
    ...
    "/api/twitter/tweet/{tweet_id}/live-room-stream",
    ...
    let platforms = Platform::ALL
        ...
        .map(|platform| {
            ...
            PlatformSummary {
                ...
                method_count: client.operations().len(),
...
/// Return the published API spec for every platform.
pub async fn api_spec(State(state): State<AppState>) -> Json<ApiSpecResponse> {
    let platforms = Platform::ALL
        .into_iter()
        .filter(|platform| state.is_platform_published(*platform))
        .map(|platform| platform_api_spec_response(platform, &state))
        .collect();

    Json(ApiSpecResponse {
        version: state.version,
        platforms,
    })
}

/// Return the published API spec for one platform.
pub async fn platform_api_spec_by_name(
    Path(platform): Path<String>,
    State(state): State<AppState>,
) -> ApiSpecResult<PlatformApiSpecResponse> {
    let parsed = platform.parse::<Platform>().map_err(|_| {
        (
            StatusCode::NOT_FOUND,
            Json(ApiSpecErrorResponse {
    ...
    Ok(Json(platform_api_spec_response(parsed, &state)))
}

fn platform_api_spec_response(platform: Platform, state: &AppState) -> PlatformApiSpecResponse {
    let spec = state.platform_client(platform).api_spec();
    response_from_api_spec(spec)
}

fn response_from_api_spec(spec: PlatformApiSpec) -> PlatformApiSpecResponse {
    PlatformApiSpecResponse {
