...
/// API spec payload for every platform.
#[derive(Debug, Serialize)]
pub struct ApiSpecResponse {
    /// Service version used to build the API spec.
    pub version: &'static str,
    /// Every published platform API spec.
    pub platforms: Vec<PlatformApiSpecResponse>,
}

/// API spec payload for one platform.
#[derive(Debug, Serialize)]
pub struct PlatformApiSpecResponse {
    /// Platform this API spec belongs to.
    pub platform: Platform,
    /// Shared API base path for the platform.
    pub api_base_path: &'static str,
    /// Number of published methods for the platform.
    ...
    pub methods: Vec<ApiOperationSpec>,
}

/// Error payload returned for invalid API spec requests.
#[derive(Debug, Serialize)]
pub struct ApiSpecErrorResponse {
