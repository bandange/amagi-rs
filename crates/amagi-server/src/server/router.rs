...
/// Build the web router for metadata, API spec, and data endpoints.
pub fn build(state: AppState) -> Router {
    Router::new()
        ...
        .route("/api/spec", get(handlers::api_spec))
        .route(
            "/api/spec/{platform}",
            get(handlers::platform_api_spec_by_name),
        )
        .nest(
...
fn platform_disabled_response(platform: Platform) -> Response<Body> {
    (
        ...
        Json(handlers::ApiSpecErrorResponse {
