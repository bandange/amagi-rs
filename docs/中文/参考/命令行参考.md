...
| 任务 | 说明 | 状态 | 必填位置参数 | 可选参数 |
| --- | --- | --- | --- | --- |
| `parse-work` | 解析单个抖音作品 | 2.0发布待测试 | `<aweme_id>` | 无 |
| `video-work` | 获取单个抖音视频作品 | 2.0发布待测试 | `<aweme_id>` | 无 |
| `image-album-work` | 获取单个抖音图文作品 | 2.0发布待测试 | `<aweme_id>` | 无 |
| `slides-work` | 获取单个抖音图集作品 | 2.0发布待测试 | `<aweme_id>` | 无 |
| `text-work` | 获取单个抖音文字作品 | 2.0发布待测试 | `<aweme_id>` | 无 |
| `work-comments` | 获取单个抖音作品的评论列表 | 2.0发布待测试 | `<aweme_id>` | `--number <u32>`，`--cursor <u64>` |
| `comment-replies` | 获取单个抖音评论的回复列表 | 2.0发布待测试 | `<aweme_id> <comment_id>` | `--number <u32>`，`--cursor <u64>` |
| `user-profile` | 获取单个抖音用户资料 | 2.0发布待测试 | `<sec_uid>` | 无 |
| `user-video-list` | 获取单个抖音用户的视频列表 | 2.0发布待测试 | `<sec_uid>` | `--number <u32>`，`--max-cursor <string>` |
| `user-favorite-list` | 获取单个抖音用户的收藏列表 | 2.0发布待测试 | `<sec_uid>` | `--number <u32>`，`--max-cursor <string>` |
| `user-recommend-list` | 获取单个抖音用户的推荐列表 | 2.0发布待测试 | `<sec_uid>` | `--number <u32>`，`--max-cursor <string>` |
| `search` | 搜索抖音内容 | 2.0发布待测试 | `<query>` | `--type <search_type>`，`--number <u32>`，`--search-id <string>` |
| `suggest-words` | 获取抖音搜索联想词 | 2.0发布待测试 | `<query>` | 无 |
| `music-info` | 获取抖音音乐信息 | 2.0发布待测试 | `<music_id>` | 无 |
| `live-room-info` | 获取抖音直播间信息 | 2.0发布待测试 | `<room_id>` | `--web-rid <string>` 必填命名参数 |
| `login-qrcode` | 请求抖音登录二维码 | 2.0发布待测试 | 无 | `--verify-fp <string>` |
| `emoji-list` | 获取抖音表情列表 | 2.0发布待测试 | 无 | 无 |
| `dynamic-emoji-list` | 获取抖音动态表情列表 | 2.0发布待测试 | 无 | 无 |
| `danmaku-list` | 获取抖音弹幕列表 | 2.0发布待测试 | `<aweme_id>` | `--duration <u64>` 必填，`--start-time <u64>`，`--end-time <u64>` |
