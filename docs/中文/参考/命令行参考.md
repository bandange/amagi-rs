...
| 任务 | 说明 | 状态 | 测试时间 | 必填位置参数 | 可选参数 |
| --- | --- | --- | --- | --- | --- |
| `search-tweets` | 搜索 Twitter/X 推文 | 测试通过 | 2026-06-17 22:23:38 CST | `<query>` | `--search-type <mode>`，`--count <u32>`，`--cursor <string>` |
| `search-users` | 搜索 Twitter/X 用户 | 测试通过 | 2026-06-17 22:23:38 CST | `<query>` | `--count <u32>`，`--cursor <string>` |
| `user-profile` | 获取单个 Twitter/X 用户资料 | 测试通过 | 2026-06-17 22:23:38 CST | `<screen_name>` | 无 |
| `user-timeline` | 获取单个 Twitter/X 用户时间线 | 测试通过 | 2026-06-17 22:23:38 CST | `<screen_name>` | `--count <u32>`，`--cursor <string>` |
| `user-replies` | 获取单个 Twitter/X 用户回复流 | 测试通过 | 2026-06-17 22:23:38 CST | `<screen_name>` | `--count <u32>`，`--cursor <string>` |
| `user-media` | 获取单个 Twitter/X 用户媒体流 | 测试通过 | 2026-06-17 22:23:38 CST | `<screen_name>` | `--count <u32>`，`--cursor <string>` |
| `user-followers` | 获取单个 Twitter/X 用户粉丝列表 | 测试通过 | 2026-06-17 22:23:38 CST | `<screen_name>` | `--count <u32>`，`--cursor <string>` |
| `user-following` | 获取单个 Twitter/X 用户关注列表 | 测试通过 | 2026-06-17 22:23:38 CST | `<screen_name>` | `--count <u32>`，`--cursor <string>` |
| `user-likes` | 获取当前已登录 Twitter/X 账户的点赞列表 | 测试通过 | 2026-06-17 22:23:38 CST | 无 | `--count <u32>`，`--cursor <string>` |
| `user-bookmarks` | 获取当前已登录 Twitter/X 账户的书签列表 | 测试通过 | 2026-06-17 22:23:38 CST | 无 | `--count <u32>`，`--cursor <string>` |
| `user-followed` | 获取当前已登录 Twitter/X 账户的关注时间线 | 测试通过 | 2026-06-17 22:23:38 CST | 无 | `--count <u32>`，`--cursor <string>` |
| `user-recommended` | 获取当前已登录 Twitter/X 账户的推荐时间线 | 测试通过 | 2026-06-17 22:23:38 CST | 无 | `--count <u32>`，`--cursor <string>` |
| `live-room-info` | 查询 Twitter/X 账户当前直播状态和直播间信息 | 测试通过 | 2026-06-17 22:23:38 CST | `<screen_name>` 可选 | `--user-id <user_id>`；`screen_name` 与 `--user-id` 二选一 |
| `live-room-stream` | 解析 Twitter/X 直播间 master HLS 播放流 | 测试通过 | 2026-06-17 22:23:38 CST | `<broadcast_id>` 可选 | `--media-key <media_key>` 或 `--tweet-id <tweet_id>`；三种输入必须且只能选一个 |
| `tweet-detail` | 获取单条 Twitter/X 推文详情 | 测试通过 | 2026-06-17 22:23:38 CST | `<tweet_id>` | 无 |
| `tweet-replies` | 获取单条 Twitter/X 推文的回复列表 | 测试通过 | 2026-06-17 22:23:38 CST | `<tweet_id>` | `--cursor <string>`，`--sort-by <sort_by>` |
| `tweet-likers` | 获取点赞单条 Twitter/X 推文的用户列表 | 测试通过 | 2026-06-17 22:23:38 CST | `<tweet_id>` | `--count <u32>`，`--cursor <string>` |
| `tweet-retweeters` | 获取转推单条 Twitter/X 推文的用户列表 | 测试通过 | 2026-06-17 22:23:38 CST | `<tweet_id>` | `--count <u32>`，`--cursor <string>` |
| `space-detail` | 获取单个 Twitter/X Space 详情 | 2.0发布测试跳过：缺少 `AMAGI_PRIVATE_TWITTER_SPACE_ID` | 2026-06-17 22:23:38 CST | `<space_id>` | 无 |
...
| 任务 | 说明 | 状态 | 测试时间 | 必填位置参数 | 可选参数 |
| --- | --- | --- | --- | --- | --- |
| `home-feed` | 获取小红书首页推荐流 | 测试通过 | 2026-06-18 00:11:45 CST | 无 | `--cursor-score <string>`，`--num <u32>`，`--refresh-type <u32>`，`--note-index <u32>`，`--category <string>`，`--search-key <string>` |
| `note-detail` | 获取单个小红书笔记详情 | 测试通过 | 2026-06-18 00:11:45 CST | `<note_id>` | `--xsec-token <string>` 必填命名参数 |
| `note-comments` | 获取单个小红书笔记的评论列表 | 测试通过 | 2026-06-18 00:11:45 CST | `<note_id>` | `--xsec-token <string>` 必填，`--cursor <string>` |
| `user-profile` | 获取单个小红书用户资料 | 测试通过 | 2026-06-18 00:11:45 CST | `<user_id>` | `--xsec-token <string>` 必填，`--xsec-source <string>` 可选 |
| `user-note-list` | 获取单个小红书用户的笔记列表 | 测试通过 | 2026-06-18 00:11:45 CST | `<user_id>` | `--xsec-token <string>` 必填，`--xsec-source <string>` 可选，`--cursor <string>`，`--num <u32>` |
| `emoji-list` | 获取小红书表情列表 | 测试通过 | 2026-06-18 00:11:45 CST | 无 | 无 |
| `search` | 搜索小红书笔记 | 测试通过 | 2026-06-18 00:11:45 CST | `<keyword>` | `--page <u32>`，`--page-size <u32>`，`--sort <sort>`，`--note-type <note_type>` |
