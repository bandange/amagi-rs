# Amagi-rs API Spec 参考

英文版：[../../en/reference/api-catalog.md](../../en/reference/api-catalog.md)

`catalog` feature 名称作为兼容入口保留。它暴露的是 Amagi-rs 的静态 API spec 元数据：
...
| 类型 | 说明 |
| --- | --- |
| `HttpMethod` | API spec 条目声明的 HTTP 方法 |
| `Platform` | 已支持平台枚举：`bilibili`、`douyin`、`kuaishou`、`twitter`、`xiaohongshu` |
| `ParsePlatformError` | 解析未知平台字符串时返回的错误 |
| `ApiOperationSpec` | 单个已发布 API 操作的元数据 |
| `PlatformApiSpec` | 单个平台的完整 API 操作元数据 |
| `ApiMethodSpec` | `ApiOperationSpec` 的兼容别名 |
| `PlatformSpec` | `PlatformApiSpec` 的兼容别名 |
...
每个 operation 条目包含以下字段：

| 字段 | 含义 |
| --- | --- |
| `method_key` | 稳定 operation key。字段名为兼容 `/api/spec` 下游解析继续保留 `method_key` |
| `chinese_name` | 从源元数据保留的中文操作名 |
...
注意：spec 中的 `route` 是规范元数据路由，不一定总是 HTTP 服务最终暴露的
完整 Axum 路由模板。如果你需要精确 HTTP 路径，请看服务接口参考文档。

## 4. Lookup Helper

| 接口 | 说明 |
| --- | --- |
| `all_platform_api_specs()` | 返回全部平台的 API spec 元数据 |
| `platform_api_spec(platform)` | 返回某个平台的 API spec 元数据 |
| `operation_specs(platform)` | 只返回某个平台的 operation 列表 |
| `find_operation(platform, operation_key)` | 按稳定 key 查找 `ApiOperationSpec` |
| `get_operation_route(platform, operation_key)` | 解析稳定 operation key 对应的 spec 路由 |
| `get_fetcher_name(platform, chinese_operation)` | 把中文操作名映射到规范英文 fetcher 名 |
| `get_chinese_operation_name(platform, fetcher_name)` | 把规范英文 fetcher 名映射回中文操作名 |

旧的 catalog/method 命名仍作为兼容包装保留：
`all_platform_specs()`、`platform_spec()`、`method_specs()`、`find_method()`、
`get_api_route()`、`get_english_method_name()`、`get_chinese_method_name()`。
...
| `twitter` | 22 | 搜索、用户关系、直播间信息、直播间播放流、推文、Space |
...
| `liveRoomInfo` | `直播间信息` | `fetchLiveRoomInfo` | `GET` | `/user/{screen_name}/live-room-info` | `user, live` | 按 `screen_name` 查询 Twitter/X 账户当前直播状态和直播间信息。 |
| `liveRoomInfoByUserId` | `按用户ID获取直播间信息` | `fetchLiveRoomInfoByUserId` | `GET` | `/user-id/{user_id}/live-room-info` | `live` | 按 numeric user id 查询 Twitter/X 账户当前直播状态和直播间信息。 |
| `liveRoomStream` | `直播间播放流` | `fetchLiveRoomStream` | `GET` | `/live-room/{broadcast_id}/stream` | `live, stream` | 按 broadcast id 解析 Twitter/X 直播间 master HLS 播放流。 |
| `liveRoomStreamByMediaKey` | `按媒体Key获取直播间播放流` | `fetchLiveRoomStreamByMediaKey` | `GET` | `/live-media/{media_key}/stream` | `live, stream` | 按 media key 解析 Twitter/X 直播间 master HLS 播放流。 |
| `liveRoomStreamByTweetId` | `按推文ID获取直播间播放流` | `fetchLiveRoomStreamByTweetId` | `GET` | `/tweet/{tweet_id}/live-room-stream` | `live, stream` | 从推文 broadcast 卡片解析 Twitter/X 直播间 master HLS 播放流。 |
