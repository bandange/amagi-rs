# Amagi-rs Rust API 使用参考

英文版：[../../en/reference/rust-api.md](../../en/reference/rust-api.md)

本文档说明 `client` feature 暴露出的稳定 Rust 客户端使用面，重点覆盖 Rust API
使用方会直接调用的接口：
...
这些内容仍然保留在 rustdoc 中；本文档聚焦“如何使用 Rust API 能力面”。

## 1. 启用 Rust API

如果你只想把本项目作为 Rust API 依赖，而不引入 CLI 或 Axum HTTP 服务：
...
`client` feature 会自动启用静态 API spec 能力。旧的 `catalog` feature 名称仍作为兼容别名保留。
...
| 项 | 类型 | 作用 |
| --- | --- | --- |
| `ClientOptions` | struct | Rust API 顶层配置，包含 Cookie 与共享请求设置 |
| `CookieConfig` | struct | 各平台 Cookie 存储 |
| `RequestConfig` | struct | 超时、重试和自定义请求头覆盖 |
| `RequestProfile` | struct | 平台默认值与用户覆盖值合并后的最终请求配置 |
| `AmagiClient` | struct | Rust API 主入口 |
| `AdapterContext` | struct | 平台级轻量客户端视图 |
| `PlatformClient` | type alias | `AdapterContext` 的兼容别名 |
...
| `AmagiClient::platform(platform)` | 为任意已支持平台构建 `AdapterContext` |
| `AmagiClient::api_specs()` | 获取全部平台的静态 API spec 元数据 |
| `AmagiClient::catalog()` | `api_specs()` 的兼容包装方法 |
| `AmagiClient::bilibili()` / `douyin()` / `kuaishou()` / `twitter()` / `xiaohongshu()` | 平台级 `AdapterContext` 快捷方法 |
...
`AdapterContext` 适合在不立即构造 fetcher 的情况下，先查看 API spec 元数据与最终请求配置。

| 接口 | 说明 |
| --- | --- |
| `AdapterContext::has_cookie()` | 返回当前平台是否绑定了非空 Cookie |
| `AdapterContext::api_base_path()` | 返回稳定的 Web 基础路径，例如 `/api/douyin` |
| `AdapterContext::api_spec()` | 返回当前适配器完整的 `PlatformApiSpec` |
| `AdapterContext::operations()` | 返回当前适配器发布的全部 `ApiOperationSpec` |
| `AdapterContext::spec()` | `api_spec()` 的兼容包装方法 |
| `AdapterContext::methods()` | `operations()` 的兼容包装方法 |
| `AdapterContext::request_profile()` | 返回最终生效的超时、重试、方法与请求头配置 |
...
## 11. 如何选择不同 Rust API 使用面

以下场景更适合使用 `AmagiClient`：

- 需要一个统一的共享配置对象
- 需要访问 API spec 元数据与事件总线
- 单进程内会调用多个平台
...
以下场景更适合直接使用静态 API spec helper：

- 只需要 operation 查找和路由元数据
