# Amagi-rs Service API Reference

Chinese version: [../../中文/参考/服务接口参考.md](../../中文/参考/服务接口参考.md)
...
## 1. Enable And Run The Service Surface

If you embed the HTTP service in your own Rust application:
...
The service runtime reads the same shared environment variables as the Rust API:
...
- Metadata endpoints return typed wrapper payloads such as `RootResponse`,
  `HealthResponse`, `ApiSpecResponse`, and `PlatformApiSpecResponse`.
...
| `GET` | `/api/spec` | none | `ApiSpecResponse` | Full static API spec metadata for all platforms |
| `GET` | `/api/spec/{platform}` | path: `platform` | `PlatformApiSpecResponse` or `ApiSpecErrorResponse` | Static API spec metadata for one platform |
