# Amagi-rs Rust API Usage Reference

Chinese version: [../../中文/参考/Rust API 参考.md](../../中文/参考/Rust API 参考.md)

This document describes the stable Rust client usage surface exposed by the `client`
feature. It focuses on the interfaces that Rust API consumers call directly:

- Client bootstrap and environment loading
- Shared request configuration
- Platform-scoped accessors
- Rust-native fetchers for every migrated platform
- Bound fetcher constructors for embedding the Rust API without a full client
...
operational Rust API surface.

## 1. Enable the Rust API

Use the crate as a Rust API without pulling in the CLI or Axum web server:
...
The `client` feature already enables the static API spec surface. The older
`catalog` feature name remains available as a compatibility alias.
...
Supported Rust API environment variables:
...
## 3. Core Rust API Interfaces

### 3.1 Shared Types

| Item | Kind | Purpose |
| --- | --- | --- |
| `ClientOptions` | struct | Top-level Rust API configuration: cookies plus shared request settings |
...
| `AmagiClient` | struct | Main Rust API entrypoint |
| `AdapterContext` | struct | Lightweight platform-scoped client view |
| `PlatformClient` | type alias | Compatibility alias for `AdapterContext` |
...
| `AmagiClient::platform(platform)` | Build a `AdapterContext` for any supported platform |
| `AmagiClient::api_specs()` | Return the complete static API spec metadata for all platforms |
| `AmagiClient::catalog()` | Compatibility wrapper for `api_specs()` |
| `AmagiClient::bilibili()` / `douyin()` / `kuaishou()` / `twitter()` / `xiaohongshu()` | Platform-specific `AdapterContext` shortcuts |
...
`AdapterContext` is useful when you want API spec metadata and the resolved
request profile without constructing a fetcher immediately.

| Interface | Description |
| --- | --- |
| `AdapterContext::has_cookie()` | Report whether a non-empty cookie is bound |
| `AdapterContext::api_base_path()` | Return the stable web base path, such as `/api/douyin` |
| `AdapterContext::api_spec()` | Return the full `PlatformApiSpec` for the adapter |
| `AdapterContext::operations()` | Return the published `ApiOperationSpec` list for the adapter |
| `AdapterContext::spec()` | Compatibility wrapper for `api_spec()` |
| `AdapterContext::methods()` | Compatibility wrapper for `operations()` |
| `AdapterContext::request_profile()` | Return the resolved timeout, retry, method, and headers |
...
assert!(!douyin.operations().is_empty());
```
...
- `new(client: AdapterContext)`
- `from_cookie(cookie, request)`
- `request_profile()`

## 6. Bilibili Rust API Interfaces
...
## 8. Kuaishou Rust API Interfaces
...
## 9. Twitter / X Rust API Interfaces
...
## 10. Xiaohongshu Rust API Interfaces
...
## 11. Choosing Between Rust API Surfaces

Use `AmagiClient` when:

- you want one shared configuration object
- you need access to API spec metadata and the event bus
...
Use the static API spec helpers when:

- you need operation lookup and route metadata only
