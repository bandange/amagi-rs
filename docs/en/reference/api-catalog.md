# Amagi-rs API Spec Reference

Chinese version: [../../中文/参考/接口规格参考.md](../../中文/参考/接口规格参考.md)

The `catalog` feature name is retained for compatibility. It exposes Amagi-rs
static API spec metadata:

- no upstream HTTP requests
- no cookies required
- no runtime fetcher construction required

Use it when you need to inspect published operations, map stable operation keys
to routes, or build tooling such as documentation generators, dashboards,
adapters, or code generation pipelines.

## 1. Enable The API Spec Surface

```toml
[dependencies]
amagi = { version = "0.1.6", default-features = false, features = ["catalog"] }
```
...
```rust
use amagi::{
    Platform, all_platform_api_specs, find_operation, get_operation_route, operation_specs,
};

let douyin_operations = operation_specs(Platform::Douyin);
assert!(!douyin_operations.is_empty());

let search = find_operation(Platform::Douyin, "search").unwrap();
assert_eq!(search.fetcher_name, "searchContent");
assert_eq!(get_operation_route(Platform::Douyin, "search"), Some("/search"));

let all = all_platform_api_specs();
assert_eq!(all.len(), 5);
```
...
| Type | Description |
| --- | --- |
| `HttpMethod` | Published HTTP verb for an API spec entry |
...
| `ApiOperationSpec` | One published API operation entry |
| `PlatformApiSpec` | Complete API operation metadata for one platform |
| `ApiMethodSpec` | Compatibility alias for `ApiOperationSpec` |
| `PlatformSpec` | Compatibility alias for `PlatformApiSpec` |
...
### 3.2 `ApiOperationSpec`

Each operation entry contains:

| Field | Meaning |
| --- | --- |
| `method_key` | Stable operation key. The field name stays `method_key` for `/api/spec` compatibility |
| `chinese_name` | Original Chinese operation label preserved from the source metadata |
| `fetcher_name` | Canonical English fetcher method name |
| `route` | Spec-relative route |
| `http_method` | Published HTTP verb |
| `description` | Short human-readable description |
| `tags` | Grouping tags such as `user`, `work`, `emoji`, or `search` |

Important: spec `route` is the canonical metadata route, not always the full
concrete Axum path template. If you need the exact HTTP path exposed by the web
server, use the Service API reference.

## 4. Lookup Helpers

| Interface | Description |
| --- | --- |
| `all_platform_api_specs()` | Return API spec metadata for every platform |
| `platform_api_spec(platform)` | Return API spec metadata for one platform |
| `operation_specs(platform)` | Return only the operation list for one platform |
| `find_operation(platform, operation_key)` | Find one `ApiOperationSpec` by stable key |
| `get_operation_route(platform, operation_key)` | Resolve the spec route for a stable operation key |
| `get_fetcher_name(platform, chinese_operation)` | Map the original Chinese label to the canonical English fetcher name |
| `get_chinese_operation_name(platform, fetcher_name)` | Map the canonical English fetcher name back to the Chinese label |

Compatibility wrappers remain available with the former catalog/method names:
`all_platform_specs()`, `platform_spec()`, `method_specs()`, `find_method()`,
`get_api_route()`, `get_english_method_name()`, and
`get_chinese_method_name()`.

## 5. Platform Coverage Summary

| Platform | Operation Count | Coverage |
| --- | --- | --- |
